...
let longitude = 0;
let searchRequest = "";
let mockEmptySearchResponse = [];
let searchCity = "";

const searchInputElement = document.getElementById("search-input");
searchInputElement.addEventListener("keydown", fetchSearchData);
...
async function fetchData() {
    if (searchRequest) {
        const searchResponse = await fetch("./mockData/search.json");
        const searchData = await searchResponse.json();

        console.log(mockEmptySearchResponse);   
        console.log(mockEmptySearchResponse[0]);
        if(!mockEmptySearchResponse[0]){
    ...
    const response = await fetch("./mockData/weather.json");
    const airResponse = await fetch("./mockData/air_pollution.json");
